...
function AddMindMapDialog({ onMindMapAdded }: { onMindMapAdded: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    addDocumentNonBlocking(collection(firestore, 'study_materials'), newMindMap);
    
    toast({
      title: 'Mind Map Added',
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Fill in the details for the new mind map. Click save when you're done.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          <Button onClick={handleSubmit}>Save Mind Map</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
export default function AdminMindMapsPage() {
  ...
  const mindMapsQuery = useMemoFirebase(() => {
    return query(collection(firestore, 'study_materials'), where('contentType', '==', 'mindmap'));
  }, [firestore]);

  const { data: mindMaps, isLoading, error } = useCollection<MindMap>(mindMapsQuery);

  const handleVisibilityChange = (mindMapId: string, newVisibility: boolean) => {
    const mindMapRef = doc(firestore, 'study_materials', mindMapId);
    updateDocumentNonBlocking(mindMapRef, { visible: newVisibility });
  };

  const handleDelete = (mindMapId: string) => {
    const mindMapRef = doc(firestore, 'study_materials', mindMapId);
    deleteDocumentNonBlocking(mindMapRef);
    toast({
  ...
  return (
    <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
       <div className="flex items-center">
          <h1 className="text-lg font-semibold md:text-2xl font-headline">Mind Maps</h1>
          <div className="ml-auto flex items-center gap-2">
            <AddMindMapDialog onMindMapAdded={() => {}} />
          </div>
        </div>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage your mind maps. You can toggle visibility, edit, or delete them.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {/* Desktop View */}
