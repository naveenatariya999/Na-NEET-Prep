      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="grid gap-4 md:hidden">
              ...
              {!isLoading && videos?.length === 0 && <p className="text-center">No videos found.</p>}
              {videos?.map((video) => (
                <Card key={video.id} className="w-full">
                  <CardHeader className="flex flex-row items-start justify-between gap-4 pb-2">
                      <div>
                        <CardTitle className="text-lg leading-tight">{video.title}</CardTitle>
                        <p className="text-sm text-muted-foreground pt-1">{video.createdAt ? video.createdAt.toDate().toLocaleDateString() : 'N/A'}</p>
                      </div>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button aria-haspopup="true" size="icon" variant="ghost">
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuLabel>Actions</DropdownMenuLabel>
                           ...
                           <DropdownMenuItem disabled>Edit</DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem onSelect={() => handleDelete(video.id)} className="text-destructive">
                            Delete
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                  ...
                  <CardFooter className="flex items-center justify-between">
                      <Label htmlFor={`visible-${video.id}`} className="text-sm text-muted-foreground">Visible</Label>
                      <Switch
                        id={`visible-${video.id}`}
                        checked={video.visible}
                        onCheckedChange={(checked) => handleVisibilityChange(video.id, checked)}
